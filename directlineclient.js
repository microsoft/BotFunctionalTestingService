...
DirectLineClient.prototype.sendMessage = function(conversationId, message, customDirectlineDomain) {
    ...
    if (isValidMessage(message)) {
        var postMessageOptions = {
            method: "POST",
            url: getConversationUrl(conversationId, customDirectlineDomain), // CodeQL [SM04580] this is a closed api that is only accessible to an internal testing service, so the ssrf risk is mitigated
...
DirectLineClient.prototype.pollMessages = function(conversationId, nMessages, bUserMessageIncluded, maxTimeout, customDirectlineDomain) {
    ...
    var getMessagesOptions = {
        method: "GET", 
        url: getConversationUrl(conversationId, customDirectlineDomain) + (this.watermark[conversationId] ? "?watermark=" + this.watermark[conversationId] : ""), // CodeQL [SM04580] this is a closed api that is only accessible to an internal testing service, so the ssrf risk is mitigated
        headers: self.headers[conversationId]
    };
